...
ob_start();
	...
	if(isset($_POST["btn_pay"]))
	{
		...
		$bk="select * from tbl_booking b inner join tbl_cart c on c.booking_id=b.booking_id INNER JOIN tbl_tool t on t.tool_id=c.tool_id where b.booking_id='".$_GET["id"]."'";
    $rowbk=($conn->query($bk)->fetch_assoc());
    $pr = $rowbk["tool_rentprice"];
    $qty = $rowbk["cart_qty"];
    $date = $rowbk["booking_delvdate"];
    ...
    $tot = ((int)$pr * (int)$qty * (int)$numberDays)-$rowbk["booking_amount"];
    $rqry="update tbl_booking set booking_status='7', booking_retrndate='".date('Y-m-d')."', booking_totalprice='".$tot."'  where booking_id='".$_GET["id"]."'";
    if($conn->query($rqry))
    {
        ?>
        <script>
            alert("Payment Complete & Return processing");
...
<head>
	...
	<title>TOOLPOOL - Payment Gateway</title>
</head>
<body>
	<div class="wrapper">
