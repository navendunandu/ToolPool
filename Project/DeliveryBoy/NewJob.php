...
<body>
    ...
    <?php
  $viewqry="select * from tbl_booking b inner join tbl_cart c on c.booking_id=b.booking_id inner join tbl_tool t on t.tool_id=c.tool_id INNER JOIN tbl_user u on u.user_id=b.user_id INNER JOIN tbl_toolshop s on t.toolshop_id=s.toolshop_id where boy_id='".$_SESSION['boyid']."' and booking_status=4 or booking_status=5";
    ...
    while($row=$result->fetch_assoc())
	  {
        ...
        { ?>
            <a href="NewJob.php?bid=<?php echo $row['booking_id']?>" class="btn btn-primary">Accept</a> <?php
        }
        else
        { ?>
            <a href="NewJob.php?did=<?php echo $row['booking_id']?>" class="btn btn-primary">Delivered</a> <?php
        } ?>
        </td>
        </tr>
        <?php } ?>
