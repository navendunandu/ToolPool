...
if(isset($_POST["btnchange"]))
{
	...
	$selqry="select * from tbl_deliveryboy where boy_id='".$_SESSION["boyid"]."'";
	$ress=$conn->query($selqry);
	$row=$ress->fetch_assoc();
	if($newpw==$repw && $oldpw==$row["boy_password"])
	{
		$pwqry="update tbl_deliveryboy set boy_password='".$newpw."' where boy_id='".$_SESSION["boyid"]."'";
		if($conn->query($pwqry))
		{
