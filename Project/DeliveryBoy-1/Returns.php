...
if(isset($_GET["pid"]))
{
    $updqry="update tbl_booking set booking_status='8' where booking_id='".$_GET["pid"]."'";
    echo $updqry;
    if($conn->query($updqry))
    {
        ?><script>alert ('Order Picked')</script>";<?php
		header("Location:../DeliveryBoy/Returns.php");
    }
    else
    {
...
<body>
        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-secondary navbar-dark">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="Returns.php" class="nav-item nav-link active"><i class="fa fa-chart-bar me-2"></i>Returns</a>
                </div>
            </nav>
        </div>
        <!-- Sidebar End -->
    ...
    <?php
  $viewqry="select * from tbl_booking b inner join tbl_cart c on c.booking_id=b.booking_id inner join tbl_tool t on t.tool_id=c.tool_id INNER JOIN tbl_user u on u.user_id=b.user_id INNER JOIN tbl_toolshop s on t.toolshop_id=s.toolshop_id where booking_status=7 and boy_id='".$_SESSION['boyid']."'";
    ...
    while($row=$result->fetch_assoc())
	  {
        ...
        <td>
            <a href="Returns.php?pid=<?php echo $row['booking_id']?>" class="btn-primary">Picked Up</a>
